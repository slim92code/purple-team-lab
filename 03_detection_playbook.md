...
```
purple-team-lab/
├── README.md / README.sr.md               # pregled (EN/SR), 2-min scan
├── 01_lab_setup.md                        # lab arhitektura
├── 02_attack_playbook.md                  # 8 faza napada
├── 03_detection_playbook.md               # detection cards + SPL (ovaj fajl)
├── 04_video_narrative.md                  # video narativ
├── lab_issues.md                          # build/debug log
├── commands.md                            # brza referenca komandi
├── config/
│   └── sysmonconfig.xml                   # Sysmon (+ C:\Temp NetworkConnect fix)
└── detections/
    ├── README.md
    ├── sigma/
    │   └── sigma_rules.yml                # 9 Sigma pravila (Detection-as-Code)
    └── splunk/
        ├── purple_lab/                    # deployable Splunk app
        │   ├── default/{app,savedsearches,props}.conf
        │   └── metadata/default.meta
        └── cim-acceleration/              # datamodels.conf + macros.conf za Splunk_SA_CIM
```
...
3. **Postavi GitHub repo** sa strukturom iznad (README + `detections/sigma/` + `detections/splunk/purple_lab/` su već tu)
